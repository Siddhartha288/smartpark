...
// Fetch car park availability summary (simulated with sample data)
$parks = [
    ['name' => 'Sydney CBD Parking Centre',    'suburb' => 'Sydney',         'total' => 80, 'occupied' => 54, 'rate' => 8.00],
    ['name' => 'Parramatta Westfield Parking', 'suburb' => 'Parramatta',     'total' => 60, 'occupied' => 18, 'rate' => 5.00],
    ['name' => 'Bondi Junction Carpark',       'suburb' => 'Bondi Junction', 'total' => 40, 'occupied' => 38, 'rate' => 7.00],
    ['name' => 'Chatswood Station Parking',    'suburb' => 'Chatswood',      'total' => 50, 'occupied' => 22, 'rate' => 4.50],
    ['name' => 'Newtown Community Parking',    'suburb' => 'Newtown',        'total' => 30, 'occupied' => 10, 'rate' => 4.00],
];
...
<section class="hero">
  <div class="hero-inner">
    <div class="hero-text">
      <h1 class="hero-title">
        Find &amp; Reserve<br><span>Parking</span> in Seconds
      </h1>
      <p class="hero-subtitle">
        SmartPark is a secure, real-time urban parking management system. 
        Search available spots across the city, reserve online, and never 
    ...
    <div class="hero-visual">
      <div class="hero-visual-title">🟢 Live Availability — Sydney CBD</div>
      <div class="hero-spot-grid" id="heroSpotGrid">
        <?php
        // Generate 24 demo spots for the hero section
        $heroSpots = [
          'available','available','occupied','available','available','occupied',
  ...
  <div class="container">
    <div class="grid-4">
      <div class="stat-card">
        ...
        <div>
          <div class="stat-value" data-stat="<?= $totalSpots ?>"><?= $totalSpots ?></div>
      ...
      <div class="stat-card">
        ...
        <div>
          <div class="stat-value" data-stat="<?= $totalAvail ?>"><?= $totalAvail ?></div>
      ...
      <div class="stat-card">
        ...
        <div>
          <div class="stat-value" data-stat="<?= count($parks) ?>"><?= count($parks) ?></div>
      ...
      <div class="stat-card">
        ...
        <div>
          <div class="stat-value" data-stat="5">5</div>
          <div class="stat-label">Sydney Suburbs Covered</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="container">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:1.25rem;">
      <div>
        ...
        <p class="text-muted">Real-time overview across all registered car parks. Last updated: <?= date('g:i A') ?></p>
      </div>
      <a href="search.php" class="btn btn-dark btn-sm">View All →</a>
    </div>
          ...
          <tbody>
            <?php foreach ($parks as $park):
              ...
              $badge = $pct > 50
                ...
                : ($pct > 20 ? '<span class="badge badge-warning">Limited</span>' : '<span class="badge badge-danger">Almost Full</span>');
            ?>
              <tr>
                ...
                <td><?= $avail ?> / <?= $park['total'] ?> <?= $badge ?></td>
                <td><strong>$<?= number_format($park['rate'], 2) ?></strong>/hr</td>
                <td>
                  <a href="search.php?suburb=<?= urlencode($park['suburb']) ?>" class="btn btn-dark btn-sm">Reserve</a>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
...
<section class="section" style="background:white;border-top:1px solid var(--border);border-bottom:1px solid var(--border);">
  <div class="container text-center">
    ...
    <p class="text-muted mt-1" style="max-width:520px;margin:0 auto 2rem;">Getting a parking spot has never been easier. Three simple steps.</p>
    ...
    <div class="grid-3">
      ...
      <div class="card" style="text-align:center;">
        <div style="font-size:2.5rem;margin-bottom:0.75rem;">✅</div>
        <h3>3. Confirm</h3>
        <p class="text-muted mt-1">Review your booking summary and confirm. A booking reference is saved to your dashboard.</p>
      </div>
    </div>
  ...
  <div class="container">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:3rem;align-items:center;">
      <div>
        ...
        <p class="text-muted mt-1" style="line-height:1.8;">
          SmartPark follows OWASP Top 10 guidelines and the Australian Privacy Act 1988. 
        ...
        <ul style="list-style:none;margin-top:1.25rem;display:flex;flex-direction:column;gap:0.65rem;">
          <li style="display:flex;align-items:center;gap:0.6rem;"><span style="color:var(--success);font-size:1.1rem;">🔐</span> Passwords hashed with bcrypt (cost 12)</li>
          <li style="display:flex;align-items:center;gap:0.6rem;"><span style="color:var(--success);font-size:1.1rem;">🛡</span> CSRF protection on all state-changing actions</li>
          <li style="display:flex;align-items:center;gap:0.6rem;"><span style="color:var(--success);font-size:1.1rem;">🔒</span> SQL injection prevented via PDO prepared statements</li>
          <li style="display:flex;align-items:center;gap:0.6rem;"><span style="color:var(--success);font-size:1.1rem;">👤</span> Role-based access control (RBAC)</li>
          <li style="display:flex;align-items:center;gap:0.6rem;"><span style="color:var(--success);font-size:1.1rem;">🌐</span> HTTPS encrypted data transmission</li>
          <li style="display:flex;align-items:center;gap:0.6rem;"><span style="color:var(--success);font-size:1.1rem;">♿</span> WCAG 2.1 Level AA accessible design</li>
        </ul>
      </div>
