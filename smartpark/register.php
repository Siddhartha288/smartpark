 ...
 * POST logic runs BEFORE header.php outputs HTML.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($_POST['csrf_token']) || !verifyCsrfToken($_POST['csrf_token'])) {
        ...
        if (empty($formErrors)) {
            // Production: hash & insert into DB
...
<div style="background:linear-gradient(135deg,var(--primary) 0%,#0d2440 100%);padding:3rem 0;">
  <div class="container-sm text-center">
    ...
    <p style="color:rgba(255,255,255,0.75);margin-top:0.4rem;">Free to join &mdash; find and reserve parking across Sydney</p>
  </div>
</div>
...
<div class="container-sm" style="padding-top:2rem;padding-bottom:3rem;">
    ...
    <form method="POST" action="register.php" id="registerForm" novalidate>
      ...
      <div class="form-group">
        ...
        <input type="text" id="name" name="name"
               ...
               class="<?= isset($formErrors['name']) ? 'error' : '' ?>" required>
      ...
      <div class="form-group">
        ...
        <input type="email" id="email" name="email"
               ...
               class="<?= isset($formErrors['email']) ? 'error' : '' ?>" required>
      ...
      <div class="form-row">
        <div class="form-group">
          <label for="password">Password * <span class="text-muted">(min 8 chars)</span></label>
          <input type="password" id="password" name="password"
                 autocomplete="new-password"
                 class="<?= isset($formErrors['password']) ? 'error' : '' ?>"
                 oninput="updatePwdStrength(this.value)" required>
        ...
        <div class="form-group">
          ...
          <input type="password" id="confirm_password" name="confirm_password"
                 autocomplete="new-password"
                 class="<?= isset($formErrors['confirm_password']) ? 'error' : '' ?>" required>
      ...
      <div class="alert alert-info" style="font-size:0.82rem;">
        <span>&#x1F512;</span>
        <span>Your data is protected under the Australian Privacy Act 1988. We only collect what is needed to provide this service.</span>
      </div>

      <!-- CAPTCHA-style checkbox -->
      <div class="captcha-box" style="<?= !empty($formErrors['captcha']) ? 'border-color:var(--danger);' : '' ?>">
        <input type="checkbox" class="captcha-check" id="captcha_confirm" name="captcha_confirm">
        <label for="captcha_confirm" class="captcha-label">I am not a robot</label>
        <div class="captcha-logo">&#x1F6E1; Verify</div>
      </div>
      <?php if (!empty($formErrors['captcha'])): ?>
        <div class="field-error show" style="margin-bottom:0.75rem;"><?= h($formErrors['captcha']) ?></div>
      <?php endif; ?>
  ...
  <p class="text-center text-muted mt-2">
    Already have an account? <a href="login.php"><strong>Log In &rarr;</strong></a>
  </p>
</div>

<script>
// Enable submit only after CAPTCHA ticked
...
function updatePwdStrength(pwd) {
    ...
    label.textContent    = score ? 'Strength: ' + labels[score] : '';
}
</script>
