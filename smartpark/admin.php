...
// FIX: $zones moved here to top-level scope so the POST handler can modify it
// and the table below will reflect those changes.
$zones = [
...
// Handle POST (add/edit park or zone)
$adminMsg = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (!isset($_POST['csrf_token']) || !verifyCsrfToken($_POST['csrf_token'])) {
    ...
    if (isset($_POST['add_park'])) {
      ...
      // FIX: Handle the add_zone form submission.
      // Find the park name that matches the submitted park_id.
      $parkName = '';
      foreach ($parks as $p) {
        if ($p['id'] == $_POST['park_id']) {
          $parkName = $p['name'];
          break;
        }
      }
      // Build the new zone and prepend it so it appears at the top of the table.
      $newZone = [
        'id'    => count($zones) + 1,
        'park'  => $parkName,
        'label' => h($_POST['zone_label']),
        'spots' => (int)$_POST['spot_count'],
        'rate'  => (float)$_POST['rate_per_hour'],
      ];
      array_unshift($zones, $newZone);
      $adminMsg = 'success:Zone "' . h($_POST['zone_label']) . '" added successfully.';
    }
  }
}
?>
