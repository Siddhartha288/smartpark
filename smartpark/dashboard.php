...
// All bookings - master list (simulating what would come from DB)
$allUpcoming = [
...
// Initialise session storage for cancelled bookings
if (!isset($_SESSION['cancelled_bookings'])) {
    $_SESSION['cancelled_bookings'] = [];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_booking'])) {
    if (empty($_POST['csrf_token']) || !verifyCsrfToken($_POST['csrf_token'])) {
        ...
        // Check it actually exists and isn't already cancelled
        $found = false;
        ...
        if ($found && !in_array($cancelledId, $_SESSION['cancelled_bookings'])) {
            // Save cancelled ID to session so it persists across page visits
            $_SESSION['cancelled_bookings'][] = $cancelledId;
...
// Build current upcoming list — exclude anything in session cancelled list
$upcomingBookings = array_values(array_filter(
    $allUpcoming,
    fn($b) => !in_array($b['id'], $_SESSION['cancelled_bookings'])
));

// Build past list — include original past bookings + any cancelled upcoming ones
$cancelledBookings = [];
foreach ($allUpcoming as $b) {
    if (in_array($b['id'], $_SESSION['cancelled_bookings'])) {
        $b['status'] = 'cancelled';
        $cancelledBookings[] = $b;
    }
}
$pastBookings = array_merge($allPast, $cancelledBookings);

$totalSpent    = array_sum(array_column(array_merge($upcomingBookings, $allPast), 'total'));
$totalBookings = count($upcomingBookings) + count($allPast);
          ...
          <?php foreach ($upcomingBookings as $b): ?>
            <div class="card" style="border-left:4px solid var(--success);">
              ...
              <div style="display:flex;gap:0.5rem;margin-top:0.85rem;">
                <form method="POST" action="dashboard.php" onsubmit="return confirm('Are you sure you want to cancel this booking?');" style="display:inline;">
                  <?= csrfField() ?>
          ...
          <table>
            ...
            <tbody>
              <?php foreach ($pastBookings as $b):
                $dur = formatDuration(strtotime($b['end']) - strtotime($b['start'])); ?>
                <tr>
                  <td>#<?= $b['id'] ?></td>
      ...
      <div class="grid-2" style="align-items:start;gap:1.5rem;">
        ...
        <div class="card">
          ...
          <div style="border-top:1px solid var(--border);margin-top:1.25rem;padding-top:1rem;">
            ...
            <a href="contact.php" class="btn btn-danger btn-sm mt-1">Request Account Deletion</a>
          </div>
        </div>
      </div>
    </div>
